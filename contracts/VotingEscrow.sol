...
/// @title Staking (modified from Origin Staking for ZIP)
...
contract VotingEscrow is ERC20Votes, ReentrancyGuard {
    ...
    IERC20 public immutable zip; // Must not allow reentrancy
    ...
    constructor(IERC20 zipToken) ERC20("Voting Escrow ZIP", "veZIP") ERC20Permit("veZIP") {
        zip = zipToken;
    ...
    /// @notice Stake zip to an address that may not be the same as the
    ...
    /// epoch.
    ///
    /// @param amount zip to lockup in the stake
    ...
    function _stake(uint256 amount, uint256 duration, address to, bool doDelegation) internal nonReentrant {
        ...
        zip.safeTransferFrom(msg.sender, address(this), amount); // Important that it's sender

        emit Stake(to, lockups[to].length - 1, amount, end, points);
    }

    /// @notice Collect staked zip for a lockup.
    ...
    function unstake(uint256 lockupId) external nonReentrant {
        ...
        zip.safeTransfer(msg.sender, amount);
    ...
    /// @param amount of additional zip to lockup. amount can be zero
    /// @param doDelegation is bool; if true, delegate to sender
    function extend(uint256 lockupId, uint256 duration, uint256 amount, bool doDelegation) external nonReentrant {
        ...
        if (amount != 0) zip.safeTransferFrom(msg.sender, address(this), amount); // Important that it's sender
